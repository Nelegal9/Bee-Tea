...
import com.alekhin.beetea.chat.domain.ConnectionResult
...
class BluetoothControllerImpl(private val context: Context): BluetoothController {
    ...
    private val bluetoothStateReceiver = BluetoothStateReceiver { isConnected, bluetoothDevice ->
        if (bluetoothAdapter?.bondedDevices?.contains(bluetoothDevice) == true) _isConnected.update { isConnected }
        else CoroutineScope(Dispatchers.IO).launch { _errors.emit("Can't connect to a non-paired device.") }
    }
    ...
    override fun startBluetoothServer(): Flow<ConnectionResult> {
        return flow {
            ...
            var shouldLoop = true
            while (shouldLoop) {
                currentClientSocket = try { currentServerSocket?.accept() } catch (e: IOException) {
                    shouldLoop = false
                    null
                }
                ...
                currentClientSocket?.let {
                    ...
                    emitAll(service.listenForIncomingMessages().map { ConnectionResult.TransferSucceeded(it) })
                }
            }
    ...
    override fun connectToDevice(device: BluetoothDeviceDomain): Flow<ConnectionResult> {
        return flow {
            if (!hasPermission(Manifest.permission.BLUETOOTH_CONNECT)) throw SecurityException("No BLUETOOTH_CONNECT permission")
            currentClientSocket = bluetoothAdapter?.getRemoteDevice(device.address)?.createRfcommSocketToServiceRecord(UUID.fromString(SERVICE_UUID))
            stopDiscovery()
    ...
    companion object {
        const val SERVICE_UUID = "018e6acf-1e01-7c22-8c93-46c9911b0a0d" // TODO: Probably implement UUID generator.
    }
}